...
describe '投稿のテスト'do
    ...
    describe 'トップ画面(top_path)のテスト'do
      ...
      context '表示の確認'do
        it'トップ画面(top_path)に「ここはTopページです」が表示されているか'do
          expect(page).to have_content 'ここはTopページです' #テストコードでは「」ではなく''
        end
        it'top_pathが"/top"であるか'do
          expect(current_page).to eq('/top')  #ページにリンクがあるかではなく現在のパスがこのURLかの確認！！
        end
      end
    end
  ...
  describe '投稿画面のテスト'do
    ...
    context '表示の確認'do
      it'todolists_new_pathが"/todolists/new"であるか'do
        expect(current_page).to eq('/todolists/new')  #ページにリンクがあるかではなく現在のパスがこのURLかの確認！！
      end
      it'投稿ボタンが表示されているか'do
        expect(page).to have_button'投稿' #投稿ボタンの「ボタン」は不要
      end
    end
    context '投稿処理のテスト'do
  ...
  describe '詳細画面のテスト'do
    ...
    context '表示のテスト'do
      it'削除リンクが存在しているか'do #もっとシンプルに考えれば良い
    ...
    context 'リンクの遷移先の確認'do
      it'編集の遷移先は編集画面か'do
        edit_link=find_all('a')[3] #3番目のaタグってこと？
        edit_link.click
        expect(current_path).to eq('todolists/'+list.id.to_s+'/edit') #pathかpageか要注意
      end
    end
